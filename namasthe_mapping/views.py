...
# =============================================================================
# UNANI CONCEPT DETAIL VIEW WITH SWAGGER DOCUMENTATION
# =============================================================================


@extend_schema(
    summary="Get Unani Concept Details",
    description="""
    Retrieve detailed information for an Unani concept with all mapped ICD-11 terms.
    
    **Features:**
    - Complete Unani concept details (Arabic name, romanized name, references)
    - All mapped ICD-11 terms with TinyBioBERT confidence scores
    - Comprehensive mapping statistics and quality metrics
    - Advanced filtering by confidence score and validation status
    - Pagination for large mapping sets
    ...
    parameters=[
        OpenApiParameter(
            ...
            type=OpenApiTypes.INT,
            location=OpenApiParameter.PATH,
            description="Primary key ID of the Unani concept",
            required=True,
            examples=[
                OpenApiExample("Small ID", value=123),
                OpenApiExample("Large ID", value=456789),
    ...
    responses={
        200: OpenApiResponse(
            response=ConceptDetailResponseSerializer,
            description="Successful retrieval of Unani concept with all mappings and statistics",
            examples=[
                OpenApiExample(
                    "Complete Unani concept response",
                    value={
                        "concept": {
                            "id": 123,
                            "code": "UN-FEVER-001",
                            "english_name": "Fever",
                            "arabic_name": "الحمى",
                            "romanized_name": "Humma",
                            "reference": "Ibn Sina, Canon of Medicine",
                        },
                        "mapping_statistics": {
                            "total_mappings": 25,
                            "validated_mappings": 20,
    ...
    tags=["NAMASTE Concepts"],
    operation_id="getUnaniConceptDetail",
)
@api_view(["GET"])
@renderer_classes([JSONRenderer])
@cache_page(60 * 10)  # 10-minute cache
def get_unani_concept_detail(request, concept_id):
    """
    Get detailed information for an Unani concept with all ICD-11 mappings
    URL: /namasthe_mapping/unani/{concept_id}/detail/
    """
    ...
    try:
        ...
        # Get Unani concept
        try:
            unani_concept = Unani.objects.get(pk=concept_id)
        except Unani.DoesNotExist:
            return Response(
                {
                    "error": "Concept not found",
                    "message": f"Unani concept with ID {concept_id} not found",
                    "code": "CONCEPT_NOT_FOUND",
                },
                status=status.HTTP_404_NOT_FOUND,
            )
        ...
        # Get ContentType and build optimized query
        content_type = ContentType.objects.get_for_model(Unani)
        mappings_query = get_optimized_mappings_query(content_type, concept_id, filters)
        ...
        # Serialize concept and mappings
        concept_serializer = UnaniConceptSerializer(unani_concept)
        mappings_serializer = ConceptMappingDetailSerializer(
            mappings_page,
            many=True,
            context={"request": request, "include_embeddings": include_embeddings},
        ...
        logger.error(f"Error in Unani concept detail view: {str(e)}", exc_info=True)
        return Response(
            {
                "error": "Internal server error",
